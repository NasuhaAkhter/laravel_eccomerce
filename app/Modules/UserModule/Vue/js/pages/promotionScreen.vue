				...
				<div class="_box _b_radious3 _padd20">
                    <div class="_1card_top _mar_b20">
							<div class="_1card_top_left">
								<p class="_1card_top_title">Promotion Screens</p>
						...
						<div class="_table_responsive">
							<Table class="" border :columns="columns" :data="alldata.data">
                                ...
                                <template slot="action" slot-scope="{row,index }">
									<Button type="primary" shape="circle" size="small" @click="openEditModal(row,index)"> Edit </Button>
                                    <i-switch :loading="switchLoading" v-model="row.is_show" @on-change="showingChange(row,index)" :true-value="1" :false-value="0"  true-color="#13ce66" false-color="#ff4949"  size="large">
...
export default {
	...
	methods:{
		...
		validateData(){
			let flag =1
			if(this.form.image_url==null || this.form.image_url.trim()==''){
				this.error.image_url = "This field is required!"
				flag =0
			}
			return flag
		},
		...
		async showingChange(item, index ){
 			this.switchLoading = true
			let ob = {
				id:item.id,
				is_show:item.is_show
			}
			const res = await this.callApi('post','/usermodule/updatePromotionStatus',ob)
			if(res.status == 200){
                this.alldata.data[index].is_show = item.is_show
		...
		async getAllCoustomer(){
			const res = await this.callApi('get', `/usermodule/getPromotionScreens?page=${this.page}&pageSize=${this.pageSize}&str=${this.str}`)
			if(res.status==200){
