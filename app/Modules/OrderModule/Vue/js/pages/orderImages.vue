    ...
    <div class="_main_content">
		<div class="row">
            <div class="col-12 col-md-12 col-lg-12">
			 ...
			 <div class="col-12 col-md-6 col-lg-6" v-for="(item,index) in alldata.data" :key="index" style="padding: 5px;">
...
export default {
	...
	methods:{
		...
		async storeProductImages(){
			...
			const res = await this.callApi('post', '/ordermodule/storeOrderImages', this.uploadedList)
			if(res.status == 200){
		...
		async deleteProductImage(id,index){
			...
			const res = await this.callApi('post', '/ordermodule/deleteOrderImage', {id:id})
			if(res.status == 200){
		...
		// upload methods
         ...
         handleSuccess(res, file) {
             this.uploadedList.unshift({image:res.image, order_id:this.id})
        },
    ...
    async created(){
        ...
        if(this.id){
            const res = await this.callApi('get',`/ordermodule/getAllOrderImages/${this.id}`)
            if(res.status == 200){
